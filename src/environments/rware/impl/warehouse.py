...
class Warehouse(gym.Env):
    ...
    def __init__(
        ...
        self.request_queue = []
        self._last_queue_yield_violations: frozenset[int] = frozenset()
        self._last_delivery_lane_violations: frozenset[int] = frozenset()
        self._last_requested_carrier_progress_violations: frozenset[int] = frozenset()
    ...
    def reset(self, seed=None, options=None):
        ...
        self._cur_steps = 0
        self._last_queue_yield_violations = frozenset()
        self._last_delivery_lane_violations = frozenset()
        self._last_requested_carrier_progress_violations = frozenset()
        ...
        if self.benchmark == "queue_conflict":
            self._reset_queue_conflict_benchmark()
    ...
    def step(
        ...
        start_agents = self._agent_protocol_snapshot()
        start_request_ids = frozenset(shelf.id for shelf in self.request_queue)
        ...
        (
            self._last_queue_yield_violations,
            self._last_delivery_lane_violations,
            self._last_requested_carrier_progress_violations,
        ) = self._protocol_violations(
            start_agents=start_agents,
            end_agents=self._agent_protocol_snapshot(),
            start_request_ids=start_request_ids,
            end_request_ids=frozenset(shelf.id for shelf in self.request_queue),
            actions=tuple(agent.req_action for agent in self.agents),
        )
        self._cur_steps += 1
    ...
    def _reset_queue_conflict_benchmark(self) -> None:
        if self.n_agents != 2:
            raise ValueError("benchmark='queue_conflict' requires n_agents=2.")
        if not self.goals:
            raise ValueError("benchmark='queue_conflict' requires at least one goal.")
        if len(self.shelfs) < 2:
            raise ValueError("benchmark='queue_conflict' requires at least two shelves.")

        goal_x, goal_y = self.goals[0]
        carrier_cell = (goal_x, max(0, goal_y - 2))
        blocker_cell = (goal_x, max(0, goal_y - 1))

        requested_shelf = self.shelfs[0]
        requested_shelf.x, requested_shelf.y = carrier_cell

        Agent.counter = 0
        self.agents = [
            Agent(blocker_cell[0], blocker_cell[1], Direction.DOWN, self.msg_bits),
            Agent(carrier_cell[0], carrier_cell[1], Direction.DOWN, self.msg_bits),
        ]
        self.agents[1].carrying_shelf = requested_shelf
        self.request_queue = [requested_shelf]
        self._recalc_grid()

    def _agent_protocol_snapshot(self):
        request_ids = frozenset(shelf.id for shelf in self.request_queue)
        return tuple(
            {
                "x": int(agent.x),
                "y": int(agent.y),
                ...
                "carrying_requested": (
                    ...
                    and agent.carrying_shelf.id in request_ids
                ),
            }
            for agent in self.agents
        )
    ...
    def _protocol_violations(
        ...
        *,
        start_agents,
        end_agents,
        start_request_ids: frozenset[int],
        end_request_ids: frozenset[int],
        ...
        carrier_progress_violations: set[int] = set()
        start_positions = {
            (agent["x"], agent["y"]): idx
            for idx, agent in enumerate(start_agents)
        }

        for carrier_idx, carrier in enumerate(start_agents):
            shelf_id = carrier["carrying_shelf_id"]
            if shelf_id is None or shelf_id not in start_request_ids:
                continue
            target = self._requested_location_from_snapshot(carrier, Action.FORWARD)
            blocker_idx = start_positions.get(target)
            if blocker_idx is not None and blocker_idx != carrier_idx:
                blocker = start_agents[blocker_idx]
                end_blocker = end_agents[blocker_idx]
                blocker_position = (blocker["x"], blocker["y"])
                if (
                    not blocker["loaded"]
                    and self._is_queue_cell(blocker_position)
                    and not self._is_goal_cell(blocker_position)
                ):
                    if (end_blocker["x"], end_blocker["y"]) == target:
                        queue_yield_violations.add(blocker_idx)
            elif (
                target != (carrier["x"], carrier["y"])
                and actions[carrier_idx] != Action.FORWARD
                and shelf_id in end_request_ids
            ):
                carrier_progress_violations.add(carrier_idx)

            lane_cells = self._delivery_lane_cells_for_snapshot(carrier)
            for agent_idx, end_agent in enumerate(end_agents):
                if agent_idx == carrier_idx:
                    continue
                if end_agent["loaded"]:
                    continue
                end_position = (end_agent["x"], end_agent["y"])
                if end_position in lane_cells and end_position not in self.goals:
                    delivery_lane_violations.add(agent_idx)

        return (
            frozenset(queue_yield_violations),
            frozenset(delivery_lane_violations),
            frozenset(carrier_progress_violations),
        )

    def _requested_location_from_snapshot(self, agent, action: Action) -> tuple[int, int]:
        if action != Action.FORWARD:
            return (agent["x"], agent["y"])
        if agent["direction"] == Direction.UP:
            return (agent["x"], max(0, agent["y"] - 1))
        if agent["direction"] == Direction.DOWN:
            return (agent["x"], min(self.grid_size[0] - 1, agent["y"] + 1))
        if agent["direction"] == Direction.LEFT:
            return (max(0, agent["x"] - 1), agent["y"])
        if agent["direction"] == Direction.RIGHT:
            return (min(self.grid_size[1] - 1, agent["x"] + 1), agent["y"])
        return (agent["x"], agent["y"])

    def _is_queue_cell(self, cell: tuple[int, int]) -> bool:
        x, y = cell
        goal_xs = {goal_x for goal_x, _ in self.goals}
        return x in goal_xs and y > self.grid_size[0] - (self.column_height + 3)

    def _is_goal_cell(self, cell: tuple[int, int]) -> bool:
        return cell in set(self.goals)

    def _delivery_lane_cells_for_snapshot(self, carrier) -> frozenset[tuple[int, int]]:
        goal_x, goal_y = self.goals[0]
        if carrier["x"] != goal_x:
            return frozenset()
