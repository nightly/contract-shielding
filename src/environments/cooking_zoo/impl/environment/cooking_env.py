...
from ..cooking_world.cooking_world import CookingWorld
from ..cooking_world.world_objects import *
from ..cooking_world.actions import *
from ..cooking_book.recipe_drawer import RECIPES, NUM_GOALS, RECIPE_STORE, DEFAULT_NUM_GOALS
...
from .game.graphic_pipeline import GraphicPipeline
...
class CookingEnvironment(AECEnv):
    ...
    def state(self):
        ...
        objects.update(self.world.world_objects)
        objects["Agent"] = self.world.agents
        width = max(float(self.world.width), 1.0)
        height = max(float(self.world.height), 1.0)
        ...
        for name, num in self.world.meta_object_information.items():
            ...
            for obj in objects[ClassToString[cls]]:
                ...
                if len(features) >= 2:
                    features[0] = features[0] / width
                    features[1] = features[1] / height
                state_vector.extend(features)
                current_num += 1
            state_vector.extend([0] * (num - current_num) * cls.feature_vector_length())
    ...
    def reset(self, seed=None, return_info=False, options=None):
        if seed is not None:
            self.seed(seed)
        options = {"full_reset": True, **(options or {})}
